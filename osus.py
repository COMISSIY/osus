...
rad = 15
class Circle:
	def __init__(self, pos=[w//2, h//2], speed=1, n=0):
		global whole_count
		whole_count += 1
		self.anim = 100
		self.rad = 50
...
while True:
	...
	for i in pygame.event.get():
		if i.type == pygame.QUIT:
			print(catched)
			exit()
		if i.type == pygame.MOUSEBUTTONUP or i.type == pygame.KEYUP:
			for i in circles:
				i.cheak_collide()
			rad = 20
		else:
	...
	if pygame.mouse.get_focused():
		...
		del_list = []
		if len(circles) < 1:
			circles.append(Circle(pos=[random.randint(75, w-75), random.randint(25, h-25)], n=len(circles)+1))
		score = f1.render(str(catched), 1, (255, 255, 255))
		accurate = f1.render(str((whole_count/catched))[0:4], 1, (255, 255, 255))
		sc.blit(score, (0, 0))
